...
const CONTRACT_ADDRESS = (process.env.NEXT_PUBLIC_CONTRACT_ADDRESS || "0xba968fA5d5255d6D95bD23D69bA63De13ceFF731") as `0x${string}`;
if (!CONTRACT_ADDRESS) {
...
const NFT_COLLECTION = [
  { 
    ...
    tier: "Legendary", 
    supply: 6969, 
    image: "https://ipfs.io/ipfs/QmaxJiJ3RQSDvuHNw5DearPFLdU8cA2L5dxDd9UWMwLUex/0.jpeg", 
    tagline: "The Ultimate Protocol Access",
    desc: "Pass utama yang diluncurkan dalam ekosistem MRxLOLCAT. Memberikan hak istimewa dalam pengambilan keputusan protokol, alokasi token $SHELL, dan akses ke node infrastruktur." 
  }
];
...
export default function MRxLOLCATBaseApp() {
  ...
  const shareToWarpcast = () => {
    const text = `I just secured my ${selectedNft.name} on Base! 🚀 Join the wave at @mrxlolcat`;
  ...
  const handleMint = useCallback(() => {
    ...
    // ERC-1155 EditionDrop claim logic
    writeContract({
      address: CONTRACT_ADDRESS,
      abi: ABI,
      functionName: 'claim',
      ...
      <header className="sticky top-0 z-50 px-3 py-2.5 backdrop-blur-xl border-b border-white/5 bg-[#020617]/80 flex items-center justify-between">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('explore')}>
          ...
          <div className="text-left">
            <h1 className="font-black text-xs tracking-tighter text-white leading-none">MRxLOLCAT</h1>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 text-left">
      <section className="space-y-2 pt-1 border-b border-white/5 pb-6 text-left">
        ...
        <h1 className="text-4xl font-black italic tracking-tighter text-white leading-none uppercase text-left">
          MRxLOLCAT <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary text-left">GENESIS.</span>
        </h1>
        <p className="text-slate-400 text-[10px] font-medium italic leading-tight max-w-[280px] text-left">
          Portal aset digital resmi MRxLOLCAT di jaringan Base. Minting Utility NFT untuk membuka akses penuh ke protokol.
        </p>
      </section>
...
function AiChatView() {
  const [messages, setMessages] = useState([
    { role: 'bot', text: 'Greetings, Voyager! I am **Clawdbot**, the neural guide for **MRxLOLCAT GENESIS**. How can I assist your journey through the protocol today?' }
  ]);
  ...
  const KNOWLEDGE_BASE: Record<string, string> = {
    'genesis': 'The **MRxLOLCAT Genesis Pass** is our legendary tier asset. It grants 100% governance rights and the highest multiplier for $SHELL allocations.',
